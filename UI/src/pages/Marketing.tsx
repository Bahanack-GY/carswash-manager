...
import { useAdminBonusSummary } from '@/api/commercial/queries'
...
type TabKey = 'clients' | 'campaigns' | 'templates' | 'promotions' | 'primes'
...
export default function Marketing() {
  ...
  /* ─── Primes tab state ─── */
  const [primesFrom, setPrimesFrom] = useState('')
  const [primesTo,   setPrimesTo]   = useState('')
  ...
  /* ─── Primes queries ─── */
  const { data: bonusSummary, isLoading: bonusLoading } = useAdminBonusSummary(
    primesFrom || primesTo ? { from: primesFrom || undefined, to: primesTo || undefined } : undefined,
  )
  ...
  const tabs: { key: TabKey; label: string; icon: React.ElementType }[] = [
    ...
    { key: 'primes', label: 'Primes', icon: Banknote },
  ]
  ...
  return (
    <motion.div variants={stagger} initial="hidden" animate="show" className="space-y-6">
      ...
      {/* ═══════════════════════════════════════════════════════════════ */}
      {/* TAB 5 — PRIMES (Commissions + Frais de service)               */}
      {/* ═══════════════════════════════════════════════════════════════ */}
      {activeTab === 'primes' && (
        <motion.div key="primes" variants={stagger} initial="hidden" animate="show" className="space-y-5">

          <motion.div variants={rise} className="flex items-center gap-2">
            <Banknote className="w-5 h-5 text-teal-500" />
            <div>
              <h2 className="font-heading text-lg font-bold text-ink">Commissions & Frais de service</h2>
              <p className="text-sm text-ink-muted">Récapitulatif des primes commerciaux et laveurs</p>
            </div>
          </motion.div>

          {/* ─ Date filters ─ */}
          <motion.div variants={rise} className="bg-panel border border-edge rounded-2xl p-4">
            <p className="text-xs font-semibold text-ink-faded uppercase tracking-wider mb-3 flex items-center gap-1.5">
              <Calendar className="w-3.5 h-3.5" /> Période
            </p>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-[11px] text-ink-muted mb-1 block">Du</label>
                <input type="date" value={primesFrom} onChange={e => setPrimesFrom(e.target.value)}
                  className="w-full px-3 py-2 bg-inset border border-edge rounded-xl text-ink text-sm outline-none focus:border-teal-500 transition-colors" />
              </div>
              <div>
                <label className="text-[11px] text-ink-muted mb-1 block">Au</label>
                <input type="date" value={primesTo} onChange={e => setPrimesTo(e.target.value)}
                  className="w-full px-3 py-2 bg-inset border border-edge rounded-xl text-ink text-sm outline-none focus:border-teal-500 transition-colors" />
              </div>
            </div>
            {(primesFrom || primesTo) && (
              <button onClick={() => { setPrimesFrom(''); setPrimesTo('') }}
          ...
          {bonusLoading ? (
            <motion.div variants={rise} className="flex justify-center py-10">
            ...
            <>
              ...
              <motion.div variants={rise} className="grid grid-cols-2 gap-4">
                <div className="bg-panel border border-edge rounded-2xl p-5">
                  <p className="text-xs text-ink-faded uppercase tracking-wide font-semibold mb-1">Commissions commerciaux</p>
                  <p className="text-2xl font-bold text-ink font-heading">
                    {(bonusSummary?.totalCommissions ?? 0).toLocaleString('fr-FR')}
                    <span className="text-sm font-normal text-ink-muted ml-1">FCFA</span>
                  </p>
                  <p className="text-xs text-ink-muted mt-1">{bonusSummary?.commercialCommissions.length ?? 0} commercial{(bonusSummary?.commercialCommissions.length ?? 0) !== 1 ? 's' : ''}</p>
                </div>
                <div className="bg-panel border border-edge rounded-2xl p-5">
                  <p className="text-xs text-ink-faded uppercase tracking-wide font-semibold mb-1">Frais de service laveurs</p>
                  <p className="text-2xl font-bold text-ink font-heading">
                    {(bonusSummary?.totalFraisService ?? 0).toLocaleString('fr-FR')}
                    <span className="text-sm font-normal text-ink-muted ml-1">FCFA</span>
                  </p>
                  <p className="text-xs text-ink-muted mt-1">{bonusSummary?.washerFrais.length ?? 0} laveur{(bonusSummary?.washerFrais.length ?? 0) !== 1 ? 's' : ''}</p>
                </div>
              </motion.div>
              ...
              <motion.div variants={rise} className="bg-panel border border-edge rounded-2xl overflow-hidden">
                <div className="px-5 py-4 border-b border-divider flex items-center gap-2">
                  ...
                  <h3 className="font-heading font-semibold text-ink text-sm">Commissions par commercial</h3>
                </div>
                {!bonusSummary || bonusSummary.commercialCommissions.length === 0 ? (
                  <div className="px-5 py-8 text-center text-ink-muted text-sm">Aucune commission sur cette période</div>
                ) : (
                  <div className="divide-y divide-divider">
                    {bonusSummary.commercialCommissions.map((c, i) => (
                      <div key={c.id} className="px-5 py-4 flex items-center gap-4">
                        <div className="w-8 h-8 rounded-xl flex items-center justify-center text-xs font-bold shrink-0"
                          style={{ background: 'oklch(0.55 0.15 180 / 0.12)', color: 'oklch(0.55 0.15 180)' }}>
                          {i + 1}
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-semibold text-ink">{c.prenom} {c.nom}</p>
                          <p className="text-xs text-ink-muted">{c.count} passage{c.count !== 1 ? 's' : ''} confirmé{c.count !== 1 ? 's' : ''}</p>
                        </div>
                        <p className="text-sm font-bold text-ok shrink-0">
                          {c.total.toLocaleString('fr-FR')} <span className="text-xs font-normal text-ink-muted">FCFA</span>
                        </p>
                      </div>
                    ))}
                  </div>
              ...
              <motion.div variants={rise} className="bg-panel border border-edge rounded-2xl overflow-hidden">
                <div className="px-5 py-4 border-b border-divider flex items-center gap-2">
                  ...
                  <h3 className="font-heading font-semibold text-ink text-sm">Frais de service par laveur</h3>
                </div>
                {!bonusSummary || bonusSummary.washerFrais.length === 0 ? (
                  <div className="px-5 py-8 text-center text-ink-muted text-sm">Aucun frais de service sur cette période</div>
                ) : (
                  <div className="divide-y divide-divider">
                    {bonusSummary.washerFrais.map((w, i) => (
                      <div key={w.id} className="px-5 py-4 flex items-center gap-4">
                        <div className="w-8 h-8 rounded-xl flex items-center justify-center text-xs font-bold shrink-0"
                          style={{ background: 'oklch(0.75 0.15 85 / 0.12)', color: 'oklch(0.65 0.15 85)' }}>
                          {i + 1}
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-semibold text-ink">{w.prenom} {w.nom}</p>
                          <p className="text-xs text-ink-muted">{w.vehiculesLaves} véhicule{w.vehiculesLaves !== 1 ? 's' : ''} lavé{w.vehiculesLaves !== 1 ? 's' : ''}</p>
                        </div>
                        <p className="text-sm font-bold text-warn shrink-0">
                          {w.totalFrais.toLocaleString('fr-FR')} <span className="text-xs font-normal text-ink-muted">FCFA</span>
                        </p>
                      </div>
                    ))}
                  </div>
