...
import type { CommercialRegistration, RegisterVehicleDto, CommercialStats, PortfolioClient, TransferPortfolioDto, CommissionResult, AdminBonusSummary } from './types';
...
export const commercialApi = {
    ...
    getCommissions: async (filters?: { from?: string; to?: string }): Promise<CommissionResult> => {
        const response = await apiClient.get<CommissionResult>('/commercial/commissions', { params: filters });
        return response.data;
    },

    getAdminBonusSummary: async (filters?: { from?: string; to?: string }): Promise<AdminBonusSummary> => {
        const response = await apiClient.get<AdminBonusSummary>('/commercial/admin/bonus-summary', { params: filters });
        return response.data;
    },
};
