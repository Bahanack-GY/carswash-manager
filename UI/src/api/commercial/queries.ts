...
export const COMMERCIAL_KEYS = {
    ...
    commissions: (filters?: CommissionFilters) => [...COMMERCIAL_KEYS.all, 'commissions', filters] as const,
    adminBonusSummary: (filters?: CommissionFilters) => [...COMMERCIAL_KEYS.all, 'admin-bonus-summary', filters] as const,
};
...
export const useCommercialCommissions = (filters?: CommissionFilters) => {
    return useQuery({
        queryKey: COMMERCIAL_KEYS.commissions(filters),
        queryFn: () => commercialApi.getCommissions(filters),
    });
};

export const useAdminBonusSummary = (filters?: CommissionFilters) => {
    return useQuery({
        queryKey: COMMERCIAL_KEYS.adminBonusSummary(filters),
        queryFn: () => commercialApi.getAdminBonusSummary(filters),
    });
};
